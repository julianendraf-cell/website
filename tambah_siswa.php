...
if(isset($_POST['submit'])){
    ...
    $insert = mysqli_query($conn, "INSERT INTO siswa (nis, nama, kelas, jurusan) 
                                   VALUES ('$nis', '$nama', '$kelas', '$jurusan')");

    if($insert){
        echo "<script>alert('Data siswa berhasil ditambahkan'); window.location='data_siswa.php';</script>";
    }else{
        echo "<script>alert('Gagal menambahkan data siswa: ".mysqli_error($conn)."');</script>";
    }
}
?>
...
<title>Tambah Siswa - SPP Online</title>
